...
public:
    ...
    Neuron(int id, Type t) : id(id), value(0.0), type(t) {}
...
public:
    ...
    // Create a basic genome with inputs and outputs
...
public:
    ...
    Species(const Genome& rep) : representative(rep), bestFitness(0.0), staleness(0) {}
...
int main() {
    // Example usage
    NEAT neat(100, 3, 2);  // 100 organisms, 3 inputs, 2 outputs
    ...
    for (int generation = 0; generation < 100; generation++) {
        ...
        for (auto& genome : neat.getPopulation()) {
            ...
            std::vector<double> inputs = {0.5, 0.7, 0.1};
            std::vector<double> outputs = genome.feedForward(inputs);
            
            // Simple fitness based on output values (just an example)
            fitness = outputs[0] * 0.8 + outputs[1] * 0.2;
